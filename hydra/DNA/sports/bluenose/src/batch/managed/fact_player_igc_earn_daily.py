...
view_mapping = {
                'game_count_1' : 'game_count_1 as demo_game_count',
                'game_count_2' : 'game_count_2 as full_game_count',
                'igc_type_1_earn_count' : 'igc_type_1_earn_count as demo_vc_count',
                'igc_type_2_earn_count' : 'igc_type_2_earn_count as full_game_vc_count',
                'igc_type_3_earn_count' : 'igc_type_3_earn_count as demo_xp_count',
                'igc_type_4_earn_count' : 'igc_type_4_earn_count as full_game_xp_count',
                'igc_type_1_earn_amount' : 'igc_type_1_earn_amount as demo_vc_amount',
                'igc_type_2_earn_amount' : 'igc_type_2_earn_amount as full_game_vc_amount',
                'igc_type_3_earn_amount' : 'igc_type_3_earn_amount as demo_xp_amount',
                'igc_type_4_earn_amount' : 'igc_type_4_earn_amount as full_game_xp_amount'
            }
...
def transform(batch_df, database, spark):
    ...
    agg_columns = {
                    'game_count_1': expr("ifnull(count(distinct case when extra_info_1 = 'demo' then game_id else null end),0) as game_count_1"),
                    'game_count_2': expr("ifnull(count(distinct case when extra_info_1 = 'full game' then game_id else null end),0) as game_count_2"),
                    'igc_type_1_earn_count' : expr("ifnull(count(distinct case when extra_info_1 = 'demo' and currency_type ='VC' then transaction_id else null end),0) as igc_type_1_earn_count"),
                    'igc_type_2_earn_count' : expr("ifnull(count(distinct case when extra_info_1 = 'full game' and currency_type ='VC' then transaction_id else null end),0) as igc_type_2_earn_count"),
                    'igc_type_3_earn_count' : expr("ifnull(count(distinct case when extra_info_1 = 'demo' and currency_type ='XP' then transaction_id else null end),0) as igc_type_3_earn_count"),
                    'igc_type_4_earn_count' : expr("ifnull(count(distinct case when extra_info_1 = 'full game' and currency_type ='XP' then transaction_id else null end),0) as igc_type_4_earn_count"),
                    'igc_type_1_earn_amount' : expr("ifnull(sum( case when extra_info_1 = 'demo' and currency_type ='VC' then currency_amount else null end),0) as igc_type_1_earn_amount"),
                    'igc_type_2_earn_amount' : expr("ifnull(sum( case when extra_info_1 = 'full game' and currency_type ='VC' then currency_amount else null end),0) as igc_type_2_earn_amount"),
                    'igc_type_3_earn_amount' : expr("ifnull(sum( case when extra_info_1 = 'demo' and currency_type ='XP' then currency_amount else null end),0) as igc_type_3_earn_amount"),
                    'igc_type_4_earn_amount' : expr("ifnull(sum( case when extra_info_1 = 'full game' and currency_type ='XP' then currency_amount else null end),0) as igc_type_4_earn_amount")
                  }
