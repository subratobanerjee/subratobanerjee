...
# MAGIC %run ../../../../../../utils/ddl/managed/fact_player_character_game_status_daily
...
from pyspark.sql.functions import (
    ...
    round as pyspark_round, min as pyspark_min ,lit,struct,explode,sum as spark_sum
)
...
def extract(spark, database, environment, prev_max):
    ...
    # Progression - only columns needed for skill tree calculation
    progression_df = spark.table(f"{database}{environment}.raw.oaktelemetry_oak_player_progression_sk") \
        ...
        .where(expr("player_platform_id IS NOT NULL AND player_character_id IS NOT NULL AND points_allocated_long > 0")) \
        .select("player_platform_id", "player_character_id", "maw_time_received",
                "skill_node_id_string", "skill_branch_id_string", "points_allocated_long", "player_character", "player_id")
    ...
    context_df = spark.table(f"{database}{environment}.raw.oaktelemetry_context_data") \
        .where(expr(filter_expr)) \
        .where(expr(buildconfig_filter)) \
        .where(expr("player_primary_id_platformid IS NOT NULL AND player_character_id_string IS NOT NULL")) \
        .select("player_primary_id_platformid", "player_character_id_string", "player_primary_id_string",
    ...
    mission_df = spark.table(f"{database}{environment}.raw.oaktelemetry_mission_state") \
        ...
        .select("player_platform_id", "player_character_id", "maw_time_received", "event_value_string",
                "mission_set_string", "game_session_uvh_level_long", "mission_objective_id_string",
                "player_id")
    ...
    poa_df = spark.table(f"{database}{environment}.raw.oaktelemetry_poa_state") \
        .where(expr(filter_expr)) \
        .where(expr(buildconfig_filter)) \
        .where(expr("player_platform_id IS NOT NULL AND player_character_id IS NOT NULL")) \
        .select("player_platform_id", "player_character_id", "maw_time_received", "event_value_string",
                "poa_status_string", "poa_type_string", "player_id")

    # Boss - select only required columns and apply null filters
    boss_df = (
        spark.table(f"{database}{environment}.raw.oaktelemetry_boss_fight")
        .where(expr(filter_expr))
        .where(expr(buildconfig_filter))
        .where(expr("player_platform_id IS NOT NULL AND player_character_id IS NOT NULL"))
        .where(expr("boss_fight_name_string IS NOT NULL AND boss_fight_name_string <> ''"))
        .select(
            "player_platform_id", "player_character_id", "maw_time_received",
            "boss_fight_result_string", "boss_fight_name_string", "player_id"
        )
        .withColumn(
            "normalized_boss_fight_result_string",
            when(col("boss_fight_result_string").isNull(), None).otherwise(lower(col("boss_fight_result_string")))
        )
    )
    ...
    ffyl_df = spark.table(f"{database}{environment}.raw.oaktelemetry_ffyl") \
        ...
        .where(expr(buildconfig_filter)) \
        .where(expr("player_platform_id IS NOT NULL AND player_character_id IS NOT NULL")) \
        .select("player_platform_id", "player_character_id", "maw_time_received", "event_value_string", "player_id")
    ...
    analytics_df = spark.table(f"{database}{environment}.raw.oaktelemetry_player_analytics") \
        ...
        .where(expr(buildconfig_filter)) \
        .where(expr("player_platform_id IS NOT NULL AND player_character_id IS NOT NULL")) \
        .select("player_platform_id", "player_character_id", "maw_time_received", "event_context", "player_id")
    ...
    character_mission_stats_df = spark.table(f"{database}{environment}.managed.fact_player_character_mission_stats_daily") \
        .where(expr(f"dw_update_ts >= to_date('{prev_max}') - INTERVAL 2 DAYS")) \
        .select("player_platform_id", "player_character_id", "mission_name", "mission_type",
                "date", "is_complete")
...
def transform(spark, database, environment, progression_df, context_df, mission_df, mission_info_df, poa_df, poa_info_df, boss_df, vehicle_df, gbx_df, ffyl_df, analytics_df, logins, character_mission_stats_df, platform_id_mapping_df, session_players):
    # Base data CTE equivalent
    base_data = progression_df.select(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("cast(maw_time_received as timestamp) as timestamp_ts"),
        expr("cast(cast(maw_time_received as timestamp) as date) as date"),
        expr("skill_node_id_string"),
        expr("skill_branch_id_string"),
        expr("points_allocated_long"),
        expr("""
        CASE 
            WHEN player_character = 'Char_ExoSoldier' THEN 'Rafa'
            WHEN player_character = 'Char_Gravitar'   THEN 'Harlowe'
            WHEN player_character = 'Char_DarkSiren'  THEN 'Vex'
            WHEN player_character = 'Char_Paladin'    THEN 'Amon'
        END
    """).alias("vault_hunter")
    ).distinct()

    # Step 2: grouped_branches - Group by key fields and collect branches
    grouped_branches = base_data.groupBy(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("date"),
        expr("timestamp_ts"),
        ...
        expr("points_allocated_long as skill_node_points_allocated"),
        collect_list(
            struct(
                expr("skill_branch_id_string").alias("skill_branch_id")
            )
        ).alias("branches")
    ).select(
        expr("player_platform_id"),
        ...
        expr("timestamp_ts"),
        expr("skill_node_id_string"),
        expr("skill_node_points_allocated"),
        expr("branches"),
        expr("vault_hunter")
    )

    # Step 3: skill_tree - Further group and create skill tree snapshot
    skill_tree = grouped_branches.groupBy(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("date"),
        expr("timestamp_ts")
        ...
        collect_list(
            struct(
                expr("skill_node_id_string").alias("skill_node_id"),
                expr("skill_node_points_allocated").alias("skill_node_points_allocated"),
                expr("branches").alias("branches")
            )
        ).alias("skill_tree_snapshot"),
        expr("max(vault_hunter)").alias("vault_hunter")
    )

    # Step 4: ordered_skill_tree_intermediate - Apply window functions
    window_spec = Window.partitionBy(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("date")
    ).orderBy(expr("timestamp_ts").desc())

    ordered_skill_tree_intermediate = skill_tree.select(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("date"),
        row_number().over(window_spec).alias("row_num"),
        last(expr("skill_tree_snapshot")).over(window_spec).alias("skill_tree_snapshot"),
        expr("vault_hunter")
    )

    # Step 5: exploded_skill_tree - Filter for row_num = 1 and explode skill tree
    exploded_skill_tree = ordered_skill_tree_intermediate.where(
        expr("row_num = 1")
    ).select(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("date"),
        explode(expr("skill_tree_snapshot")).alias("skill_node"),
        expr("vault_hunter")
    )

    # Step 6: Final result - Group and aggregate with distinct
    prog = exploded_skill_tree.groupBy(
        expr("player_platform_id"),
        expr("player_character_id"),
        expr("date")
    ).agg(
        spark_sum(expr("skill_node.skill_node_points_allocated")).alias("skill_total_points_allocated"),
        collect_list(expr("skill_node")).alias("skill_tree_snapshot"),
        expr("max(vault_hunter)").alias("vault_hunter")
    ).distinct()
    ...
    ctx = (
        ...
        .join(
            days_since.alias("days_since"),
             expr("ctx.player_primary_id_platformid = days_since.player_platform_id AND CAST(ctx.maw_time_received AS DATE) = days_since.date_from_logins"),
            "left"
        )
        .groupBy(
            expr("ctx.player_primary_id_platformid AS player_platform_id"),
            expr("ctx.player_character_id_string AS player_character_id"),
            coalesce(col("ctx.player_primary_id_string"), col("ctx.player_primary_id_platformid"), lit("Unknown")).alias("player_id"),
        ...
        .agg(
            ...
            expr("ROUND(MAX(session_players.total_session_time_hrs), 2) AS total_hours_played"),
            expr("ROUND(MAX(CASE WHEN session_players.session_type LIKE '%Co-Op%' THEN session_players.total_session_time_hrs ELSE 0 END), 2) AS multiplayer_hours_played"),
            expr("ROUND(MAX(CASE WHEN session_players.session_type = 'Online Co-Op Session' THEN session_players.total_session_time_hrs ELSE 0 END), 2) AS online_multiplayer_hours_played"),
            expr("ROUND(MAX(CASE WHEN session_players.session_type = 'Local Co-Op Session' THEN session_players.total_session_time_hrs ELSE 0 END), 2) AS split_screen_hours_played")
        )
    )
    ...
    filtered_df = context_df \
        .filter(expr("player_primary_id_platformid IS NOT NULL AND player_character_id_string IS NOT NULL AND player_profile_difficulty_long IS NOT NULL")) \
        .withColumn("ts", expr("CAST(maw_time_received AS TIMESTAMP)"))

    agg_df = filtered_df.groupBy(
        expr("player_primary_id_platformid"),
        expr("player_character_id_string"),
        expr("ts")
    ).agg(
    ...
    player_difficulty = ranked_df \
        ...
        .selectExpr(
            ...
            "player_primary_id_platformid AS player_character_id",
    ...
    all_missions = mission_df.alias("mission") \
        ...
        .groupBy(
            ...
            expr("mission.player_character_id").alias("player_character_id"),
        ...
        .withColumn(
            "row_num",
            expr("row_number() over (partition by player_platform_id,player_character_id, cast(cast(ts as timestamp) as date) order by cast(ts as timestamp) desc, mission_step_num desc)")
        ) \
        .orderBy(
            expr("ts desc"),
    ...
    mission = mission_df.alias("mission").groupBy(
        ...
        expr("mission.player_character_id"),
    ...
    #newly adding the below logic to calculate the ltd values for main story completion and side mission completion
    ...
    window_spec_cum = Window.partitionBy("player_platform_id", "player_character_id", "mission_type").orderBy(
        ...
        .select(
            ...
            expr('''
             max(main_story_completion) OVER (
                PARTITION BY player_character_id
                ORDER BY date
   ...
   .groupBy(
    ...
    boss = boss_df.withColumn("date", expr("date(cast(maw_time_received as timestamp))")) \
        .groupBy("player_platform_id", "player_character_id", "date") \
        .agg(
            expr("SUM(CASE WHEN LOWER(boss_fight_result_string) = 'success' AND boss_fight_name_string = 'Boss_IdolatorSol' THEN 1 ELSE 0 END)").alias("times_beat_sol"),
            expr("SUM(CASE WHEN LOWER(boss_fight_result_string) = 'success' AND boss_fight_name_string = 'Boss_VileLictor' THEN 1 ELSE 0 END)").alias("times_beat_liktor"),
            expr("SUM(CASE WHEN LOWER(boss_fight_result_string) = 'success' AND boss_fight_name_string = 'Boss_Callis_Fortress' THEN 1 ELSE 0 END)").alias("times_beat_callis_fortress"),
            expr("SUM(CASE WHEN LOWER(boss_fight_result_string) = 'success' AND boss_fight_name_string = 'Boss_Callis_Elpis' THEN 1 ELSE 0 END)").alias("times_beat_callis_elpis"),
            expr("SUM(CASE WHEN LOWER(boss_fight_result_string) = 'success' AND boss_fight_name_string = 'Boss_Timekeeper_Human' THEN 1 ELSE 0 END)").alias("times_beat_tk_human"),
            expr("SUM(CASE WHEN LOWER(boss_fight_result_string) = 'success' AND boss_fight_name_string = 'Boss_Timekeeper_Guardian' THEN 1 ELSE 0 END)").alias("times_beat_tk_guardian")
        )
    ...
    recent_boss = (boss_df
        ...
        .groupBy("player_platform_id","player_character_id", "ts")
        .agg(
            last("boss_fight_name_string").alias("most_recent_boss"),
            last("normalized_boss_fight_result_string").alias("most_recent_boss_outcome")
        )
        .withColumn(
            "row_num",
            row_number().over(
                Window.partitionBy("player_platform_id", "player_character_id", to_date("ts"))
                .orderBy(desc("ts"))
            )
        )
        .orderBy(desc("ts"))
    ...
    vehicle = vehicle_df.groupBy(
        ...
        expr("COUNT(DISTINCT CASE WHEN event_key_string = 'Begin' THEN event_guid_string ELSE NULL END)").alias("vehicle_use_count"),
        expr("ROUND(SUM(CASE WHEN event_key_string = 'End' THEN time_since_vehicle_enter_long / 60 ELSE 0 END), 2)").alias("minutes_in_vehicle")
    )
    ...
    ffyl = ffyl_df.groupBy(
        ...
        expr("SUM(CASE WHEN event_value_string IN ('Instant', 'TimerExpired', 'GiveUp') THEN 1 ELSE NULL END)").alias("times_deaths")
    )
    ...
    prog_join_condition = (
        (ctx["player_platform_id"] == prog["player_platform_id"]) &
        (ctx["player_character_id"] == prog["player_character_id"]) &
        (ctx["date"] == prog["date"])
    )
    ...
    .join(prog, prog_join_condition, "left") \
    ...
    ctx["player_character_id"].isNotNull() &
    ctx["date"].isNotNull()
    )
    ...
    ctx["player_character_id"],
    ctx["date"]
    ).orderBy(
    col("total_hours_played").desc(),
    ...
    fill_window = Window.partitionBy(ctx["player_platform_id"], ctx["player_character_id"]).orderBy(
        ctx["date"]).rowsBetween(Window.unboundedPreceding, Window.currentRow)
    ...
    # Select the required columns with proper null handling
    final_df = windowed_df.select(
    # do null values in non-core fields need to have default values?
    ctx["player_platform_id"],
    coalesce(ctx["player_id"], ctx["player_platform_id"], lit("Unknown")).alias("player_id"),
    ctx["player_character_id"],
    ctx["date"],
    prog["skill_tree_snapshot"],
    prog["skill_total_points_allocated"],
    prog["vault_hunter"],
    ...
    coalesce(ctx["total_hours_played"], lit(0)).alias("total_hours_played"),
    ...
    "merge_key",
    expr("sha2(concat_ws('|', player_platform_id ,player_id, player_platform_id, player_character_id,CAST(date AS STRING)), 256)")
    )

    return final_df

def load(final_df, database, environment, spark):
    # Reference to the target Delta table
    target_df = DeltaTable.forName(spark, f"{database}{environment}.managed.fact_player_character_game_status_daily")

    # Merge condition
    merger_condition = 'target.merge_key = source.merge_key'

    # Define field categories for update logic
    no_update_fields = ['dw_insert_ts', 'merge_key', 'player_platform_id', 'player_id', 'player_character_id', 'date']
    ...
    source_update_fields = [
        'recent_mission_name', 'recent_mission_type', 'recent_boss_name', 'recent_boss_result',
        'days_since_last_login'
    ]
    
    # Fields that should use coalesce() to fill nulls
    coalesce_fields = ['vault_hunter', 'skill_tree_snapshot', 'platform']
    ...
    insert_expr = {col: f"source.{col}" for col in final_df.columns}

    (target_df.alias("target")
     .merge(final_df.alias("source"), merger_condition)
...
def run_batch(database, environment):
    ...
    create_fact_player_character_game_status_daily(spark, database, environment)
    create_fact_player_character_game_status_daily_view(spark, database, environment)

    print('Extracting the data')

    prev_max = max_timestamp(spark, f"{database}{environment}.managed.fact_player_character_game_status_daily", 'date')
    if prev_max is None:
    ...
    #transform
    final_df=transform(spark, database, environment,progression_df, context_df, mission_df, mission_info_df, poa_df, poa_info_df, boss_df, vehicle_df, gbx_df, ffyl_df, analytics_df,logins,character_mission_stats_df,platform_id_mapping_df,session_players)
    print('Merge Data')
