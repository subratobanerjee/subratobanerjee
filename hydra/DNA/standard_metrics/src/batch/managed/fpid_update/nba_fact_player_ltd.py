...
def transform(spark, target_df):
    updated_df = (
        target_df
        .where(expr("fpid_hashed IS NULL AND title ILIKE 'NBA%' AND title != 'NBA 2K Playgrounds 2'"))
        .withColumn("fpid_hashed", expr(f"{database}{environment}.cdp_ng.salted_puid(player_id)"))
...
def load(spark, target_table, updated_df):
    ...
    delta_table.alias("old").merge(
        ...
        condition="old.fpid_hashed IS NULL AND old.title ILIKE 'NBA%' AND old.title != 'NBA 2K Playgrounds 2'",
        set={
