...
checkpointLocation = "dbfs:/tmp/horizon/intermediate/fact_player_match_summary_4"
...
spa_df = (
    spark
    .readStream
    .table("horizon_load_test.raw.collectedammo1")
...
mhp_df = (
    spark
    .readStream
    .table("horizon_load_test.raw.match_hero_progression")
    .withColumn("receivedOn", col("receivedOn").cast("timestamp"))
    .withColumn("match_id",
                when(col("event_defaults.match_type") == "Trials", col("group.id"))
                .otherwise(col("event_defaults.match_id")))
...
def proc_batch(df, batch_id):
    (
        ...
        .tableName(f"horizon_load_test.managed.fact_player_match_summary")
    ...
    target_table = DeltaTable.forName(spark, "horizon_load_test.managed.fact_player_match_summary")
