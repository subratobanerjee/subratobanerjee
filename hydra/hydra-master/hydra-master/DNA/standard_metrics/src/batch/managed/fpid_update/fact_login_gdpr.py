...
def transform(core_df, spark):
    dm_df = (
        spark.read.table(f"{database}{environment}.standard_metrics.fact_login")
        .alias("dm")
    )
...
def load(updated_df, spark):
    delta_table = DeltaTable.forName(spark, f"{database}{environment}.standard_metrics.fact_login")
