...
def extract(environment, spark):
    ...
    sql_query_seasons = f"""
    SELECT * FROM dataanalytics{environment}.cdp_ng.vw_eth_seasons;
    """
    
    # Execute the SQL query and return the result as a DataFrame
    print(sql_query_seasons)
    df = spark.sql(sql_query_seasons)
    return df
...
def load(df, environment, spark):
    ...
    df.write \
      ...
      .saveAsTable(f"dataanalytics{environment}.cdp_ng.eth_seasons")

# COMMAND ----------

def run_batch():
    print("running batch for seasonss.....")
