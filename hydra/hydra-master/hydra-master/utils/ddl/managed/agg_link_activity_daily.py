...
def create_agg_link_activity_daily(spark, database,environment,properties={}):
    """
    Create the agg_link_activity_daily table in the specified environment.
    This function constructs a SQL command to create the agg_link_activity_daily
    ...
    Output:  "dbfs:/tmp/bluenose/managed/streaming/run_dev/agg_link_activity_daily"
    """

    sql = f"""
    CREATE TABLE IF NOT EXISTS {database}{environment}.managed.agg_link_activity_daily (
        DATE DATE,
        PLATFORM STRING,
        SERVICE STRING,
        COUNTRY_CODE STRING,
    ...
    create_agg_link_activity_daily_view(spark, database, view_mapping)
    return

def create_agg_link_activity_daily_view(spark, database, mapping):
    """
    ...
    sql = f"""
    CREATE OR REPLACE VIEW {database}{environment}.managed_view.agg_link_activity_daily AS (
        SELECT
            DATE,
            PLATFORM,
            SERVICE,
            ...
            DW_INSERT_TS,
            DW_UPDATE_TS
        from {database}{environment}.managed.agg_link_activity_daily
    )
    """
