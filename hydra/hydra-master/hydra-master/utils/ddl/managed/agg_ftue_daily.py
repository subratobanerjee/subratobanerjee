...
def create_agg_ftue_daily(spark, database,view_mapping, properties={}):
    """
    ...
    sql = f"""
            CREATE TABLE IF NOT EXISTS {database}{environment}.managed.agg_ftue_daily (
                ...
                config_1 string comment 'extra config for the grain',
    ...
    create_agg_ftue_daily_view(spark, database, view_mapping)
    return f"dbfs:/tmp/{database}/managed/batch/run{environment}/agg_ftue_daily"


# COMMAND ----------

def create_agg_ftue_daily_view(spark, database, mapping):
    """
    ...
    """
    sql = f"""
    CREATE OR REPLACE VIEW {database}{environment}.managed_view.agg_ftue_daily AS (
        SELECT
            platform,
            service,
            step, 
            ...
            dw_insert_ts,
            dw_update_ts
        from {database}{environment}.managed.agg_ftue_daily
    )
    """
    spark.sql(sql)
