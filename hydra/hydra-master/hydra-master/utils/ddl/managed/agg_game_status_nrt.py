...
def create_agg_game_status_nrt(spark, database, view_mapping, properties={}):
    """
    Create the agg_game_status_nrt table in the specified environment.

    This function constructs a SQL command to create the agg_game_status_nrt
    ...
    Output:  "dbfs:/tmp/wwe2k25/managed/streaming/run_dev/agg_game_status_nrt"
    """
    
    sql = f"""
    CREATE TABLE IF NOT EXISTS {database}{environment}.managed.agg_game_status_nrt (
        timestamp_10min_slice TIMESTAMP,
        platform STRING,
        service STRING,
        country_code STRING DEFAULT 'ZZ',
        territory_name STRING DEFAULT 'ZZ',
        game_mode STRING DEFAULT 'Other',
    ...
    create_agg_game_status_nrt_view(spark, database, view_mapping)
    return f"dbfs:/tmp/{database}/managed/streaming/run{environment}/agg_game_status_nrt"

# COMMAND ----------

def create_agg_game_status_nrt_view(spark, database, mapping):
    """
    Create the agg_game_status_nrt view in the specified environment.
    ...
    sql = f"""
    CREATE view if not exists {database}{environment}.managed_view.agg_game_status_nrt AS (
        SELECT
        ...
        from {database}{environment}.managed.agg_game_status_nrt
    )
    """
...
def load_agg_game_status_nrt(spark, df, database, environment):
    """
    Load the data into the agg_game_status_nrt table in the specified environment.
    ...
    final_table = DeltaTable.forName(spark, f"{database}{environment}.managed.agg_game_status_nrt")

    # get schema of the table
    table_schema = spark.read.table(f"{database}{environment}.managed.agg_game_status_nrt").schema
    ...
    df = df.selectExpr(
            ...
            "SHA2(CONCAT_WS('|', timestamp_10min_slice, platform, service, country_code, territory_name, game_mode), 256) as merge_key"
        )
    out_df = out_df.unionByName(df, allowMissingColumns=True)

    # dynamically setup merge condition based on the aggregate columns in this database's agg_game_status_nrt table
    agg_cols = [col_name for col_name in out_df.columns if 'agg_' in col_name]
    merge_condition = f"new.agg_gp_1 > 0"
