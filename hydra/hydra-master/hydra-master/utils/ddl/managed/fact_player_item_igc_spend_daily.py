...
def create_fact_player_item_igc_spend_daily(spark, database,view_mapping, properties={}):
    """
    ...
    sql = f"""
            CREATE TABLE IF NOT EXISTS {database}{environment}.managed.fact_player_item_igc_spend_daily (
    ...
    create_fact_player_item_igc_spend_daily_view(spark, database, view_mapping)
    return f"dbfs:/tmp/{database}/managed/streaming/run{environment}/fact_player_item_igc_spend_daily"


# COMMAND ----------

def create_fact_player_item_igc_spend_daily_view(spark, database, mapping):
    """
    ...
    """
    sql = f"""
    CREATE OR REPLACE VIEW {database}{environment}.managed_view.fact_player_item_igc_spend_daily AS (
        SELECT
            date,
            player_id,
            platform,
            ...
            dw_insert_ts,
            dw_update_ts
        from {database}{environment}.managed.fact_player_item_igc_spend_daily
    )
    """
    spark.sql(sql)