...
def create_fact_player_game_status_daily(spark, database, view_mapping, properties={}):
    """
    Create the fact_player_game_status_daily table in the specified environment.

    This function constructs a SQL command to create the fact_player_game_status_daily
    ...
    CREATE TABLE IF NOT EXISTS {database}{environment}.managed.fact_player_game_status_daily (
        ...
        extra_grain_2 STRING DEFAULT 'ZZ',
    ...
    create_fact_player_game_status_daily_view(spark, database, view_mapping)
    return f"dbfs:/tmp/{database}/managed/streaming/run{environment}/fact_player_game_status_daily"

# COMMAND ----------

def create_fact_player_game_status_daily_view(spark, database, mapping):
    """
...
def load_fact_player_game_status_daily(spark, df, database, environment):
    """
