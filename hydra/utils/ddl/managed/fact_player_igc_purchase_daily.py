...
def create_fact_player_igc_purchase_daily(spark, database,view_mapping, properties={}):
    """
    ...
    sql = f"""
            CREATE TABLE IF NOT EXISTS {database}{environment}.managed.fact_player_igc_purchase_daily (
            ...
            comment 'the table will have ingame currency purchased player level aggregated data at every 10 mins'
            partitioned by (platform,dw_insert_date)
    """
    create_table(spark, sql, properties)
    create_fact_player_igc_purchase_daily_view(spark, database, view_mapping)
    return f"dbfs:/tmp/{database}/managed/batch/run{environment}/fact_player_igc_purchase_daily"


# COMMAND ----------

def create_fact_player_igc_purchase_daily_view(spark, database, mapping):
    """
    ...
    """
    sql = f"""
    CREATE VIEW IF NOT EXISTS {database}{environment}.managed_view.fact_player_igc_purchase_daily AS (
        SELECT
            date,
            player_id,
            platform,
            ...
            dw_insert_ts,
            dw_update_ts
        from {database}{environment}.managed.fact_player_igc_purchase_daily
    )
    """
    spark.sql(sql)
