...
def create_agg_mission_stats_nrt(spark, database, view_mapping, properties={}):
    """
    Create the agg_mission_stats_nrt table in the specified environment.

    This function constructs a SQL command to create the agg_mission_stats_nrt
    ...
    Output:  "dbfs:/tmp/wwe2k25/managed/streaming/run_dev/create_fact_player_session"
    """
    ...
    create_mission_stats_nrt_view(spark, database, view_mapping)
    return f"dbfs:/tmp/{database}/managed/streaming/run{environment}/agg_mission_stats_nrt"

# COMMAND ----------

def create_mission_stats_nrt_view(spark, database, mapping):
    """
    Create the mission_stats_nrt view in the specified environment.
    ...
    sql = f"""
    CREATE view if not exists {database}{environment}.managed_view.agg_mission_stats_nrt AS (
        SELECT
        ...
        from {database}{environment}.managed.agg_mission_stats_nrt
    )
    """
...
def load_agg_mission_stats_nrt(spark, df, database, environment):
    """
    Load the data into the mission_stats_nrt table in the specified environment.
    ...
    final_table = DeltaTable.forName(spark, f"{database}{environment}.managed.agg_mission_stats_nrt")

    # get schema of the table
    table_schema = spark.read.table(f"{database}{environment}.managed.agg_mission_stats_nrt").schema
    ...
    # dynamically setup merge condition based on the aggregate columns in this database's agg_mission_stats_nrt table
